...
public class ViewSdImageActivity extends AppCompatActivity implements SdApiResponseListener {
    ...
    public void callSD4Img() {
        ...
        if (mBound) {
            ...
            } else if (mCurrentSketch.getCnMode().startsWith(Sketch.CN_MODE_IDMVTON)) {
                requestType = "comfyui";
                jsonObject = sdApiHelper.getIdmVtonJSON(mCurrentSketch);
                sdBaseUrl = sharedPreferences.getString("dflApiAddress", "");
    ...
    public static void updateMBitmap() {
        ...
        if (SdParam.SD_INPUT_IMAGE_SKETCH.equals(param.baseImage)) {
            boundary *= 2;
        }
        if (mCurrentSketch.getImgPaint() != null) {
            boundary += (int)Math.round(Math.max(mCurrentSketch.getImgPaint().getWidth(), mCurrentSketch.getImgPaint().getHeight()) / 100d);
        }
        if (param.inpaintPartial == SdParam.INPAINT_PARTIAL) {
